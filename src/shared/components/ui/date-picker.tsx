...
function Calendar({
  ...
  const months = React.useMemo(() => {
    let locale: Pick<Locale, "options" | "localize" | "formatLong"> = id;
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        day: "h-9 w-9 text-center text-base p-0 relative [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20 rounded-1",
        day_button: cn(
        ...
        selected:
          "bg-primary text-primary-foreground hover:bg-accent hover:text-secondary-foreground focus:bg-primary focus:text-primary-foreground rounded-l-md rounded-r-md",
  ...
  (
    ...
    const initHourFormat = {
      hour24:
        ...
        `PPP${!granularity || granularity === "second" ? ":ss" : ""}`,
      hour12:
        displayFormat?.hour12 ??
        `PP${!granularity || granularity === "second" ? ":ss" : ""} b`,
    };
      ...
      <Popover>
        <PopoverTrigger asChild disabled={disabled}>
          <Button
            variant="outline"
            className={cn(
              "pointer-events-auto! h-10.5 w-full justify-between text-left text-base font-normal hover:bg-transparent",
              !displayDate &&
                "text-muted-foreground hover:text-muted-foreground",
              disabled &&
                "cursor-not-allowed! bg-grey-20 text-grey-70! opacity-100!",
              className,
            )}
            ref={buttonRef}
          >

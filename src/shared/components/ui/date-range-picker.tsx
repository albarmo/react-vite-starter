...
export const DateRangePicker: FC<DateRangePickerProps> = ({
  ...
  const selectedPreset = useMemo(() => {
    const normalizeDate = (date?: Date): number => {
      const normalizedDate = new Date(date ?? 0);
    ...
    for (const preset of PRESETS) {
      ...
      if (
        normalizeDate(range.from) === normalizeDate(presetRange.from) &&
        normalizeDate(range.to) === normalizeDate(presetRange.to)
      ) {
        return preset.name;
      }
    }

...
import { PublisherDeleteModal } from "@/features/master-file/authority-files/components/publisher-delete-modal";
...
function PublisherSelectionBar({
  sidebarOpen,
  onDeleteSelected,
}: SelectionBarProps) {
  return (
...
export function PublisherPage() {
  ...
  const [records, setRecords] = useState(PUBLISHER_RECORDS);
  const [deleteTarget, setDeleteTarget] = useState<DeleteTarget | null>(null);
  const [search, setSearch] = useState("");
  const [activeFilter, setActiveFilter] = useState<PublisherStatus>("active");
  ...
  const columns = useMemo<ColumnDef<PublisherRecord>[]>(
    () => [
      {
        id: "select",
        meta: {
            ...
            <div className="flex items-center justify-end gap-3">
              <Button
                ...
                onClick={(event) => {
                  ...
                  navigate(
                    `/master-file/authority-files/publisher/edit/${row.original.id}`,
                  );
                }}
              >
    ...
    >
      ...
      <MdUpwardCard className="mt-5">
        <PublisherToolbar
          ...
          onCreate={() =>
            navigate("/master-file/authority-files/publisher/create")
          }
        />
        ...
        <Spinner visible={isFiltering}>
          <DataTable
            ...
            onRowClick={(record) =>
              navigate(
                `/master-file/authority-files/publisher/detail/${record.id}`,
              )
            }
          />
        </Spinner>
      ...
      <PublisherDeleteModal
        ...
        description={
          deleteTarget?.type === "selected"
            ? "Are you sure you want to delete selected Publishers?"

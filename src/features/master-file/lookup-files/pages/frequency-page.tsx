...
import { FrequencyDeleteModal } from "@/features/master-file/lookup-files/components/frequency-delete-modal";
import {
...
function FrequencySelectionBar({
  sidebarOpen,
  onDeleteSelected,
}: SelectionBarProps) {
  return (
...
export default function FrequencyPage() {
  ...
  const [records, setRecords] = useState(FREQUENCY_PRESET_RECORDS);
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((record) => {
      ...
      return [
        record.frequency,
        getFrequencyLanguageLabel(record.language),
        record.timeIncrement,
        getFrequencyTimeUnitLabel(record.timeUnit),
        record.updatedAt,
    ...
    () => [
      {
        ...
        header: () => (
          <SelectionCheckbox
            ...
            ariaLabel="Select all frequency rows on current page"
          />
        ),
        cell: ({ row }) => (
          <div onClick={(event) => event.stopPropagation()}>
            <SelectionCheckbox
              ...
              ariaLabel={`Select ${row.original.frequency}`}
            />
          </div>
        ),
      },
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-base text-grey-90">
            {getFrequencyLanguageLabel(row.original.language)}
          </span>
        ),
      },
      {
        accessorKey: "timeIncrement",
        meta: {
          headClassName: "w-[190px]",
          cellClassName: "px-3 py-4",
        },
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-base text-grey-90">
            {getFrequencyTimeUnitLabel(row.original.timeUnit).toLowerCase()}
          </span>
        ),
      },
      {
          ...
          return (
            <div className="flex items-center justify-end gap-3">
              <Button
                ...
                size="icon-sm"
                disabled={isSelected}
                aria-label={`Edit ${row.original.frequency}`}
                className={cn(
                ...
                onClick={(event) => {
                  ...
                  navigate(
                    `/master-file/look-up-files/frequency/edit/${row.original.id}`,
                  );
                }}
              >
              ...
              <Button
                ...
                size="icon-sm"
                disabled={isSelected}
                aria-label={`Delete ${row.original.frequency}`}
                className={cn(
    ...
    >
      ...
      <MdUpwardCard className="mt-5">
        <FrequencyToolbar
          ...
          onCreate={() => navigate("/master-file/look-up-files/frequency/create")}
        />
        ...
        <Spinner visible={isFiltering}>
          <DataTable
            ...
            onRowClick={(record) =>
              navigate(`/master-file/look-up-files/frequency/detail/${record.id}`)
            }
          />
        </Spinner>
      ...
      <FrequencyDeleteModal
        ...
        description={
          deleteTarget?.type === "selected"
            ? `Are you sure you want to delete ${deleteTarget.ids.length} selected Frequencies?`
            : undefined
        }

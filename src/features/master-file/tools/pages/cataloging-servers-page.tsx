...
import { CatalogingServerDeleteModal } from "@/features/master-file/tools/components/cataloging-server-delete-modal";
import {
...
function CatalogingServersToolbar({
  search,
  onSearchChange,
  onClearSearch,
  onCreate,
...
function CatalogingServersFooter({
  pageSize,
  totalItems,
  displayedCount,
  onPageSizeChange,
  ...
  return (
    <div className="mt-6 flex items-center justify-between gap-4">
      ...
      <p className="text-base text-grey-90">
        {totalItems === 0
          ? "0 of 0"
          : `1 - ${displayedCount} of ${totalItems}`}
      </p>
    </div>
  );
}

function CatalogingServersSelectionBar({
  sidebarOpen,
  onDeleteSelected,
}: SelectionBarProps) {
  return (
...
export default function CatalogingServersPage() {
  const navigate = useNavigate();
  const sidebarOpen = useUiStore((state) => state.sidebarOpen);
  const [records, setRecords] = useState(CATALOGING_SERVER_PRESET_RECORDS);
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((record) => {
      ...
      return [
        record.name,
        record.url,
        getCatalogingServerTypeLabel(record.type),
        record.updatedAt,
    ...
    () => [
      {
        ...
        header: () => (
          <SelectionCheckbox
            ...
            ariaLabel="Select all cataloging server rows on current page"
          />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: "URL",
        cell: ({ row }) => (
          <span className="text-base text-grey-90">{row.original.url}</span>
        ),
      },
      {
        id: "type",
        ...
        cell: ({ row }) => (
          <span className="text-base text-grey-90">
            {getCatalogingServerTypeLabel(row.original.type).replace(
              "server",
              "Servers",
            )}
          </span>
            ...
            <div className="flex items-center justify-end gap-3">
              <Button
                ...
                onClick={(event) => {
                  ...
                  navigate(
                    `/master-file/tools/cataloging-servers/edit/${row.original.id}`,
                  );
                }}
              >
  ...
  return (
    ...
    >
      <AppPageHeader title="Cataloging Servers" />

      <MdUpwardCard className="mt-5">
        <CatalogingServersToolbar
          search={search}
          ...
          onCreate={() => navigate("/master-file/tools/cataloging-servers/create")}
        />
        ...
        <Spinner visible={isFiltering}>
          <DataTable
            ...
            onRowClick={(record) =>
              navigate(`/master-file/tools/cataloging-servers/detail/${record.id}`)
            }
          />
        </Spinner>

        <CatalogingServersFooter
          pageSize={pageSize}
      ...
      <CatalogingServerDeleteModal
        ...
        description={
          deleteTarget?.type === "selected"
            ? `Are you sure you want to delete ${deleteTarget.ids.length} selected Cataloging Servers?`
            : undefined
        }

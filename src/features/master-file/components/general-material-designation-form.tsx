...
import {
  generalMaterialDesignationFormSchema,
  type GeneralMaterialDesignationFormData,
  type GeneralMaterialDesignationFormInitialState,
} from "./general-material-designation-form-presets";

type GeneralMaterialDesignationFormProps = {
  mode: "create" | "edit";
  pageTitle: string;
...
function GeneralMaterialDesignationFormBreadcrumb({
  mode,
}: {
  mode: "create" | "edit";
}) {
  return (
    <Breadcrumb className="mt-2">
      <BreadcrumbList className="text-sm text-grey-80">
        ...
        <BreadcrumbItem>
          <BreadcrumbLink asChild className="hover:text-blue-50">
            <Link to="/master-file/authority-files/gmd">Authority Files</Link>
          </BreadcrumbLink>
        </BreadcrumbItem>
        <BreadcrumbSeparator className="text-grey-70" />
        <BreadcrumbItem>
        ...
        <BreadcrumbItem>
          <BreadcrumbPage className="font-medium text-blue-50">
            {mode === "create" ? "Create GMD" : "Edit GMD"}
          </BreadcrumbPage>
        </BreadcrumbItem>
      </BreadcrumbList>
    </Breadcrumb>
...
function GeneralMaterialDesignationFormActionBar({
  sidebarOpen,
  onCancel,
}: {
  sidebarOpen: boolean;
  ...
  return (
    ...
    >
      <div className="mx-auto flex w-full max-w-310 items-center justify-end gap-3 px-3 py-4 sm:px-4 md:px-8">
        ...
        <Button type="submit" form="general-material-designation-form">
          Save
        </Button>
      </div>
    </div>
...
export function GeneralMaterialDesignationForm({
  mode,
  pageTitle,
  initialState,
  recordId,
}: GeneralMaterialDesignationFormProps) {
  ...
  const form = useForm<GeneralMaterialDesignationFormData>({
    resolver: zodResolver(generalMaterialDesignationFormSchema),
    defaultValues,
  });
  ...
  const handleCancel = () => {
    if (mode === "edit" && recordId) {
      navigate(`/master-file/authority-files/gmd/detail/${recordId}`);
      return;
    }

    navigate("/master-file/authority-files/gmd");
  };
  ...
  return (
    <PageContainer className="pt-6 pb-32 md:pt-8 md:pb-36">
      <div>
        ...
        <GeneralMaterialDesignationFormBreadcrumb mode={mode} />
      </div>

      <MdUpwardCard className="mt-6 rounded-[18px] p-5">
        <Form {...form}>
          <form
            id="general-material-designation-form"
            className="space-y-6"
      ...
      <GeneralMaterialDesignationFormActionBar

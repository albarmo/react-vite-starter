...
import { SubjectVocabularyDeleteModal } from "@/features/master-file/components/subject-vocabulary-delete-modal";
import { SubjectVocabularyControlModal } from "@/features/master-file/components/subject-vocabulary-control-modal";
...
export function SubjectForm({
  ...
  useEffect(() => {
    ...
    vocabularyControlIdSequence.current = defaultValues.vocabularyControls.length;
  }, [defaultValues, form]);

  const deferredVocabularySearch = useDeferredValue(vocabularySearch);
  const normalizedVocabularySearch =
    deferredVocabularySearch.trim().toLowerCase();
  ...
  const filteredVocabularyControls = useMemo(() => {
    ...
    return vocabularyControlRows.filter((item) =>
      [
        ...
        item.vocabulary,
        item.classification,
      ].some((value) => value.toLowerCase().includes(normalizedVocabularySearch)),
    );
  ...
  const handleVocabularyControlSubmit = (
    ...
    if (editingVocabularyControlIndex !== null) {
      ...
      update(editingVocabularyControlIndex, {
        ...
        relatedTerm: values.relatedTerm as SubjectVocabularyControl["relatedTerm"],
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex items-center gap-3 text-base text-grey-90">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setVocabularyPageSize(Number(value))}
                  >

...
import { ItemStatusDeleteModal } from "@/features/protected/library/master-file/lookup-files/components/item-status-delete-modal";
import { ITEM_STATUS_PRESET_RECORDS } from "@/features/protected/library/master-file/lookup-files/components/item-status-form-presets";
import type {
...
function ItemStatusSelectionBar({
  sidebarOpen,
  onDeleteSelected,
}: SelectionBarProps) {
  return (
...
export default function ItemStatusPage() {
  ...
  const [records, setRecords] = useState(ITEM_STATUS_PRESET_RECORDS);
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((record) => {
      ...
      return [record.code, record.name, record.rules, record.updatedAt].some(
    ...
    () => [
      {
        ...
        header: () => (
          <SelectionCheckbox
            ...
            ariaLabel="Select all item status rows on current page"
          />
        ),
        cell: ({ row }) => (
            ...
            <div className="flex items-center justify-end gap-3">
              <Button
                ...
                onClick={(event) => {
                  ...
                  navigate(
                    `/master-file/look-up-files/item-status/edit/${row.original.id}`,
                  );
                }}
              >
  ...
  return (
    ...
    >
      <AppPageHeader title="Item Status" />
      ...
      <MdUpwardCard className="mt-5">
        <ItemStatusToolbar
          ...
          onCreate={() =>
            navigate("/master-file/look-up-files/item-status/create")
          }
        />
        ...
        <Spinner visible={isFiltering}>
          <DataTable
            ...
            onRowClick={(record) =>
              navigate(
                `/master-file/look-up-files/item-status/detail/${record.id}`,
              )
            }
          />
        </Spinner>
      ...
      <ItemStatusDeleteModal
        ...
        description={
          deleteTarget?.type === "selected"
            ? "Are you sure you want to delete selected Item Statuses?"
            : "Are you sure you want to delete this Item Status?"
        }
      />
    </PageContainer>
  );

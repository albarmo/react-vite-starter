...
import {
  ITEM_STATUS_PRESET_RECORDS,
  formatItemStatusRules,
} from "@/features/protected/library/master-file/lookup-files/components/item-status-form-presets";
...
function ItemStatusDetailBreadcrumb() {
  return (
    <Breadcrumb className="mt-2">
      <BreadcrumbList className="text-sm text-grey-80">
        ...
        <BreadcrumbItem>
          <BreadcrumbLink asChild className="hover:text-blue-50">
            <Link to="/master-file/look-up-files/item-status">Item Status</Link>
          </BreadcrumbLink>
        </BreadcrumbItem>
        <BreadcrumbSeparator className="text-grey-70" />
        <BreadcrumbItem>
...
export function ItemStatusDetailPage() {
  ...
  const detail = useMemo(
    () =>
      ITEM_STATUS_PRESET_RECORDS.find((item) => item.id === id) ??
      ITEM_STATUS_PRESET_RECORDS[1] ??
  ...
  return (
    <PageContainer className="pt-6 pb-10 md:pt-8">
      ...
      <MdUpwardCard className="mt-6 rounded-[18px] p-5">
        <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <Button
              ...
              onClick={() =>
                navigate(
                  `/master-file/look-up-files/item-status/edit/${detail.id}`,
                )
              }
            >
        ...
        <dl className="mt-8 grid max-w-180 grid-cols-[104px_minmax(0,1fr)] gap-x-8 gap-y-4">
          ...
          <dt className="text-grey-90">Rules</dt>
          <dd className="text-grey-100">
            {formatItemStatusRules(detail.rules)}
          </dd>
      ...
      <ItemStatusDeleteModal
        ...
        onDelete={() => navigate("/master-file/look-up-files/item-status")}
      />
    </PageContainer>
  );
}

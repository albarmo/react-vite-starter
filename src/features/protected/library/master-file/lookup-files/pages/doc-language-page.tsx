...
import { DocLanguageDeleteModal } from "@/features/protected/library/master-file/lookup-files/components/doc-language-delete-modal";
import { DOC_LANGUAGE_PRESET_RECORDS } from "@/features/protected/library/master-file/lookup-files/components/doc-language-form-presets";
import type {
  DocLanguageDeleteTarget as DeleteTarget,
  DocLanguageRecord,
  DocLanguageFooterProps as FooterProps,
  DocLanguageSelectionBarProps as SelectionBarProps,
  DocLanguageToolbarProps as ToolbarProps,
  ...
  return (
    <div className="mt-6 flex items-center justify-between gap-4">
      ...
      <p className="text-base text-grey-90">
        {totalItems === 0
          ? "0 dari 0"
          : `1 - ${displayedCount} dari ${totalItems}`}
      </p>
    </div>
  );
}

function DocLanguageSelectionBar({
  sidebarOpen,
  onDeleteSelected,
}: SelectionBarProps) {
  return (
...
export default function DocLanguagePage() {
  const navigate = useNavigate();
  const sidebarOpen = useUiStore((state) => state.sidebarOpen);
  const [records, setRecords] = useState(DOC_LANGUAGE_PRESET_RECORDS);
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((record) => {
      ...
      return [record.language, record.updatedAt].some((value) =>
  ...
  const columns = useMemo<ColumnDef<DocLanguageRecord>[]>(
    () => [
      {
        id: "select",
        meta: {
        ...
        header: () => (
          <SelectionCheckbox
            ...
            ariaLabel="Select all doc language rows on current page"
          />
        ),
        cell: ({ row }) => (
          <div onClick={(event) => event.stopPropagation()}>
            <SelectionCheckbox
              ...
              ariaLabel={`Select ${row.original.language}`}
            />
          </div>
        ),
      },
          ...
          return (
            <div className="flex items-center justify-end gap-3">
              <Button
                ...
                size="icon-sm"
                disabled={isSelected}
                aria-label={`Edit ${row.original.language}`}
                className={cn(
                ...
                onClick={(event) => {
                  ...
                  navigate(
                    `/master-file/look-up-files/doc-language/edit/${row.original.id}`,
                  );
                }}
              >
              ...
              <Button
                ...
                size="icon-sm"
                disabled={isSelected}
                aria-label={`Delete ${row.original.language}`}
                className={cn(
  ...
  return (
    ...
    >
      <AppPageHeader title="Doc. Language" />
      ...
      <MdUpwardCard className="mt-5">
        <DocLanguageToolbar
          ...
          onCreate={() =>
            navigate("/master-file/look-up-files/doc-language/create")
          }
        />
        ...
        <Spinner visible={isFiltering}>
          <DataTable
            ...
            onRowClick={(record) =>
              navigate(
                `/master-file/look-up-files/doc-language/detail/${record.id}`,
              )
            }
          />
        </Spinner>
      ...
      <DocLanguageDeleteModal
        ...
        description={
          deleteTarget?.type === "selected"
            ? `Are you sure you want to delete ${deleteTarget.ids.length} selected Doc. Languages?`
            : undefined
        }

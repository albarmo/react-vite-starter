...
import { LABEL_PRESET_RECORDS } from "@/features/protected/library/master-file/lookup-files/components/label-form-presets";
import type {
...
} from "@/features/protected/library/master-file/lookup-files/types/label.types";
...
export default function LabelPage() {
  ...
  const [records, setRecords] = useState(LABEL_PRESET_RECORDS);
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((record) => {
      ...
      return [record.description, record.name, record.updatedAt].some((value) =>
    ...
    () => [
      {
        ...
        header: () => (
          <SelectionCheckbox
            ...
            ariaLabel="Select all label rows on current page"
          />
        ),
        cell: ({ row }) => (
            ...
            <div className="flex items-center justify-end gap-3">
              <Button
                ...
                onClick={(event) => {
                  ...
                  navigate(
                    `/master-file/look-up-files/label/edit/${row.original.id}`,
                  );
                }}
              >
    ...
    >
      ...
      <MdUpwardCard className="mt-5">
        <LabelToolbar
          ...
          onCreate={() => navigate("/master-file/look-up-files/label/create")}
        />
        ...
        <Spinner visible={isFiltering}>
          <DataTable
            ...
            onRowClick={(record) =>
              navigate(`/master-file/look-up-files/label/detail/${record.id}`)
            }
          />
        </Spinner>
      ...
      <LabelDeleteModal
        ...
        description={
          deleteTarget?.type === "selected"
            ? `Are you sure you want to delete ${deleteTarget.ids.length} selected Labels?`
            : undefined
        }

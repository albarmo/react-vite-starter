...
import { ITEM_CODE_PATTERN_PRESET_RECORDS } from "@/features/protected/library/master-file/tools/components/item-code-pattern-presets";
import type {
...
} from "@/features/protected/library/master-file/tools/types/item-code-pattern.types";
...
function ItemCodePatternToolbar({
  search,
  onSearchChange,
  onClearSearch,
  onCreate,
...
function ItemCodePatternFooter({
  pageSize,
  totalItems,
  displayedCount,
  onPageSizeChange,
...
function ItemCodePatternSelectionBar({
  sidebarOpen,
  onDeleteSelected,
}: SelectionBarProps) {
  return (
...
export default function ItemCodePatternPage() {
  const navigate = useNavigate();
  const sidebarOpen = useUiStore((state) => state.sidebarOpen);
  const [records, setRecords] = useState(ITEM_CODE_PATTERN_PRESET_RECORDS);
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((record) => {
      ...
      return [record.name, record.prefix, record.suffix, record.updatedAt].some(
    ...
    () => [
      {
        ...
        header: () => (
          <SelectionCheckbox
            ...
            ariaLabel="Select all item code pattern rows on current page"
          />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        id: "actions",
        meta: {
          headClassName: "w-[96px] text-right",
  ...
  return (
    ...
    >
      <AppPageHeader title="Item Code Pattern" />

      <MdUpwardCard className="mt-5">
        <ItemCodePatternToolbar
          search={search}
          ...
          onCreate={() =>
            navigate("/master-file/tools/item-code-pattern/create")
          }
        />
        ...
        <Spinner visible={isFiltering}>
          <DataTable
            ...
            onRowClick={(record) =>
              navigate(
                `/master-file/tools/item-code-pattern/detail/${record.id}`,
              )
            }
          />
        </Spinner>

        <ItemCodePatternFooter
          pageSize={pageSize}
      ...
      <ItemCodePatternDeleteModal
        ...
        description={
          deleteTarget?.type === "selected"
            ? `Are you sure you want to delete ${deleteTarget.ids.length} selected Item Code Patterns?`
            : undefined
        }

...
import { carrierTypeFormSchema } from "@/features/protected/library/master-file/authority-files/schema/carrier-type.schema";
import type {
  CarrierTypeFormData,
...
function CarrierTypeFormBreadcrumb({ mode }: { mode: "create" | "edit" }) {
  return (
    <Breadcrumb className="mt-2">
      <BreadcrumbList className="text-sm text-grey-80">
        ...
        <BreadcrumbItem>
          <BreadcrumbLink asChild className="hover:text-blue-50">
            <Link to="/master-file/authority-files/carrier-type">
              Authority Files
            </Link>
          </BreadcrumbLink>
        </BreadcrumbItem>
        ...
        <BreadcrumbItem>
          <BreadcrumbPage className="font-medium text-blue-50">
            {mode === "create" ? "Create Carrier Type" : "Edit Carrier Type"}
          </BreadcrumbPage>
        </BreadcrumbItem>
      </BreadcrumbList>
    </Breadcrumb>
...
function CarrierTypeFormActionBar({
  sidebarOpen,
  onCancel,
}: {
  sidebarOpen: boolean;
  ...
  return (
    ...
    >
      <div className="mx-auto flex w-full max-w-310 items-center justify-end gap-3 px-3 py-4 sm:px-4 md:px-8">
        ...
        <Button type="submit" form="carrier-type-form">
          Save
        </Button>
      </div>
    </div>
...
export function CarrierTypeForm({
  ...
  const form = useForm<CarrierTypeFormData>({
    resolver: zodResolver(carrierTypeFormSchema),
    defaultValues,
  });
  ...
  const handleCancel = () => {
    navigate("/master-file/authority-files/carrier-type");
  };
  ...
  return (
    <PageContainer className="pt-6 pb-32 md:pt-8 md:pb-36">
      <div>
        ...
        <CarrierTypeFormBreadcrumb mode={mode} />
      </div>

      <MdUpwardCard className="mt-6 rounded-[18px] p-5">
        <Form {...form}>
          <form
            id="carrier-type-form"
            className="space-y-6"

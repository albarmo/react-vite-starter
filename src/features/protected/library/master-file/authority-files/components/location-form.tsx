...
import { locationFormSchema } from "@/features/protected/library/master-file/authority-files/schema/location.schema";
import type {
  LocationFormData,
  LocationFormInitialState,
  LocationFormProps,
} from "@/features/protected/library/master-file/authority-files/types/location.types";
...
function LocationFormBreadcrumb({ mode }: { mode: "create" | "edit" }) {
  return (
...
function LocationFormActionBar({
  sidebarOpen,
  onCancel,
}: {
  sidebarOpen: boolean;
...
export function LocationForm({
  ...
  const handleCancel = () => {
    if (mode === "edit" && recordId) {
      navigate(`/master-file/authority-files/location/detail/${recordId}`);
      return;
    }

    navigate("/master-file/authority-files/location");
  };
  ...
  return (
    <PageContainer className="pt-6 pb-32 md:pt-8 md:pb-36">
      <div>
        ...
        <LocationFormBreadcrumb mode={mode} />
      </div>

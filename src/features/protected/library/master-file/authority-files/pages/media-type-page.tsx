...
import { MediaTypeDeleteModal } from "@/features/protected/library/master-file/authority-files/components/media-type-delete-modal";
import type {
...
} from "@/features/protected/library/master-file/authority-files/types/media-type.types";
...
const MEDIA_TYPE_RECORDS: MediaTypeRecord[] = Array.from(
  { length: 1000 },
  (_, index) => {
    const base =
      MEDIA_TYPE_BASE_RECORDS[index % MEDIA_TYPE_BASE_RECORDS.length];

    return {
      id: `media-type-${index + 1}`,
      code:
        index < MEDIA_TYPE_BASE_RECORDS.length
          ? base.code
          : `${base.code}-${index + 1}`,
      name: base.name,
...
function MediaTypeSelectionBar({
  sidebarOpen,
  onDeleteSelected,
}: SelectionBarProps) {
  return (
...
export function MediaTypePage() {
  const navigate = useNavigate();
  const sidebarOpen = useUiStore((state) => state.sidebarOpen);
  const [records, setRecords] = useState(MEDIA_TYPE_RECORDS);
  ...
  const columns = useMemo<ColumnDef<MediaTypeRecord>[]>(
    () => [
      {
        id: "select",
        meta: {
          ...
          return (
            <div className="flex items-center justify-end gap-3">
              <Button
                ...
                onClick={() =>
                  navigate(
                    `/master-file/authority-files/media-type/edit/${row.original.id}`,
                  )
                }
              >
  ...
  return (
    ...
    >
      <AppPageHeader title="Media Type" />
      ...
      <MdUpwardCard className="mt-5">
        <MediaTypeToolbar
          ...
          onCreate={() =>
            navigate("/master-file/authority-files/media-type/create")
          }
        />
      ...
      <MediaTypeDeleteModal
        ...
        description={
          deleteTarget?.type === "selected"
            ? "Are you sure you want to delete selected Media Types?"

...
import { CarrierTypeDeleteModal } from "@/features/protected/library/master-file/authority-files/components/carrier-type-delete-modal";
import type {
  CarrierTypeRecord,
  CarrierTypeDeleteTarget as DeleteTarget,
  CarrierTypeFooterProps as FooterProps,
  CarrierTypeSelectionBarProps as SelectionBarProps,
  CarrierTypeToolbarProps as ToolbarProps,
} from "@/features/protected/library/master-file/authority-files/types/carrier-type.types";
...
const CARRIER_TYPE_BASE_RECORDS: Omit<CarrierTypeRecord, "id">[] = [
  {
    code: "ha",
    name: "aperture card",
...
const CARRIER_TYPE_RECORDS: CarrierTypeRecord[] = Array.from(
  { length: 1000 },
  (_, index) => {
    const base =
      CARRIER_TYPE_BASE_RECORDS[index % CARRIER_TYPE_BASE_RECORDS.length];

    return {
      id: `carrier-type-${index + 1}`,
      code:
        index < CARRIER_TYPE_BASE_RECORDS.length
          ? base.code
          : `${base.code}-${index + 1}`,
      name: base.name,
...
function CarrierTypeSelectionBar({
  sidebarOpen,
  onDeleteSelected,
}: SelectionBarProps) {
  return (
...
export function CarrierTypePage() {
  const navigate = useNavigate();
  const sidebarOpen = useUiStore((state) => state.sidebarOpen);
  const [records, setRecords] = useState(CARRIER_TYPE_RECORDS);
  ...
  const columns = useMemo<ColumnDef<CarrierTypeRecord>[]>(
    () => [
      {
        id: "select",
        meta: {
          ...
          return (
            <div className="flex items-center justify-end gap-3">
              <Button
                ...
                onClick={() =>
                  navigate(
                    `/master-file/authority-files/carrier-type/edit/${row.original.id}`,
                  )
                }
              >
  ...
  return (
    ...
    >
      <AppPageHeader title="Carrier Type" />
      ...
      <MdUpwardCard className="mt-5">
        <CarrierTypeToolbar
          ...
          onCreate={() =>
            navigate("/master-file/authority-files/carrier-type/create")
          }
        />
      ...
      {selectedCount > 0 ? (
        <CarrierTypeSelectionBar
          sidebarOpen={sidebarOpen}
          onDeleteSelected={handleOpenDeleteSelected}
        />
      ) : null}

      <CarrierTypeDeleteModal
        ...
        description={
          deleteTarget?.type === "selected"
            ? "Are you sure you want to delete selected Carrier Types?"
            : "Are you sure you want to delete this Carrier Type?"
        }
      />
    </PageContainer>
  );
